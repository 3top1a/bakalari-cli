...
def get_timetable_data(server, token):
    ...
    for x in day_table_raw:
        for y in x['Atoms']:
            day_table.append( [ x['DayOfWeek'], y['HourId'], y['SubjectId'], y['TeacherId'], y['Theme'], y['RoomId'] ] )
...
def display_timetable_data(data):
...
def timetable(server, password, username):
    ...
    display_timetable_data(get_timetable_data(server, token))

# Parsing
def main():
    try:
        opts, args = getopt.gnu_getopt(sys.argv[1:], "hs:p:u:t", ["server", "password", "username", "homework"])
